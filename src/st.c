...
char *base64dec(const char *src) {
    ...
    char             *result, *dst;
    static const char base64_digits[256] = {[43] = 62,
        0,
        0,
        0,
        63,
...
int ttynew(const char *line, char *cmd, const char *out, char **args) {
    ...
    if (out) {
        ...
        if (iofd < 0) { fprintf(stderr, "Error opening %s:%s\n", out, strerror(errno)); }
    }
...
void tsetchar(Rune u, const Glyph *attr, int x, int y) {
    ...
    if (isboxdraw(u)) { term.line[y][x].mode |= ATTR_BOXDRAW; }
}
    ...
    for (lim = args + narg; args < lim; ++args) {
        if (priv) {
            switch (*args) {
                ...
                case 25: /* DECTCEM -- Text Cursor Enable Mode */ xsetmode(!set, MODE_HIDE); break;
                ...
                case 1003: /* 1003: enable all mouse motions */
                    ...
                    break;
                case 1004: /* 1004: send focus events to tty */ xsetmode(set, MODE_FOCUS); break;
                case 1006: /* 1006: extended reporting mode */ xsetmode(set, MODE_MOUSESGR); break;
                ...
                case 2004: /* 2004: bracketed paste mode */ xsetmode(set, MODE_BRCKTPASTE); break;
            ...
            switch (*args) {
                ...
                case 4: /* IRM -- Insertion-replacement */ MODBIT(term.mode, set, MODE_INSERT); break;
                case 12: /* SRM -- Send/Receive */ MODBIT(term.mode, !set, MODE_ECHO); break;
    ...
    switch (csiescseq.mode[0]) {
        ...
        case 'g': /* TBC -- Tabulation clear */
            switch (csiescseq.arg[0]) {
                ...
                case 3: /* clear all the tabs */ memset(term.tabs, 0, term.col * sizeof(*term.tabs)); break;
        ...
        case 'K': /* EL -- Clear line */
            switch (csiescseq.arg[0]) {
                case 0: /* right */ tclearregion(term.c.x, term.c.y, term.col - 1, term.c.y); break;
        ...
        case 'L': /* IL -- Insert <n> blank lines */
            ...
            break;
        case 'l': /* RM -- Reset Mode */ tsetmode(csiescseq.priv, 0, csiescseq.arg, csiescseq.narg); break;
        ...
        case 'd': /* VPA -- Move to <row> */
            ...
            break;
        case 'h': /* SM -- Set terminal mode */ tsetmode(csiescseq.priv, 1, csiescseq.arg, csiescseq.narg); break;
        case 'm': /* SGR -- Terminal attribute (color) */ tsetattr(csiescseq.arg, csiescseq.narg); break;
...
void strhandle(void) {
    ...
    switch (strescseq.type) {
        case ']': /* OSC -- Operating System Command */
            ...
            break;
        case 'k': /* old title set compatibility */ xsettitle(strescseq.args[0]); return;
...
void tcontrolcode(uchar ascii) {
    switch (ascii) {
        ...
        case '\017': /* SI (LS0 -- Locking shift 0) */ term.charset = 1 - (ascii - '\016'); return;
        case '\032': /* SUB */
        ...
        case 0x9a: /* DECID -- Identify Terminal */ ttywrite(vtiden, strlen(vtiden), 0); break;
        case 0x9b:       /* TODO: CSI */
...
int eschandle(uchar ascii) {
    switch (ascii) {
        ...
        case 'o': /* LS3 -- Locking shift 3 */ term.charset = 2 + (ascii - 'n'); break;
        ...
        case 'M':        /* RI -- Reverse index */
            ...
            break;
        case 'Z': /* DECID -- Identify Terminal */ ttywrite(vtiden, strlen(vtiden), 0); break;
        case 'c': /* RIS -- Reset to initial state */
            treset();
            resettitle();
            xloadcols();
            break;
        case '=': /* DECPAM -- Application keypad */ xsetmode(1, MODE_APPKEYPAD); break;
...
check_control_code:
    ...
    if (IS_SET(MODE_INSERT) && term.c.x + width < term.col) { memmove(gp + width, gp, (term.col - term.c.x - width) * sizeof(Glyph)); }
...
void tresize(int col, int row) {
    ...
    tmp = col;
    if (!term.maxcol) { term.maxcol = term.col; }
    ...
    c = term.c;
    for (i = 0; i < 2; i++) {
        if (mincol < col && 0 < minrow) { tclearregion(mincol, 0, col - 1, minrow - 1); }
...
void draw(void) {
    ...
    if (term.scr == 0) { xdrawcursor(cx, term.c.y, term.line[term.c.y][cx], term.ocx, term.ocy, term.line[term.ocy][term.ocx], term.line[term.ocy], term.col); }
