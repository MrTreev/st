...
void mousereport(XEvent *e) {
    ...
    if (IS_SET(MODE_MOUSESGR)) {
        len = snprintf(buf, sizeof(buf), "\033[<%d;%d;%d%c", button, x + 1, y + 1, e->type == ButtonRelease ? 'm' : 'M');
    } else if (x < 223 && y < 223) {
...
uint buttonmask(uint button) { return button == Button1 ? Button1Mask : button == Button2 ? Button2Mask : button == Button3 ? Button3Mask : button == Button4 ? Button4Mask : button == Button5 ? Button5Mask : 0; }
...
void propnotify(XEvent *e) {
    ...
    xpev                      = &e->xproperty;
    if (xpev->state == PropertyNewValue && (xpev->atom == XA_PRIMARY || xpev->atom == clipboard)) { selnotify(e); }
}
...
int ximopen(Display *dpy) {
    ...
    if (xw.ime.xic == NULL) { xw.ime.xic = XCreateIC(xw.ime.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, xw.win, XNDestroyCallback, &icdestroy, NULL); }
...
void xinit(int cols, int rows) {
    ...
    /* input methods */
    if (!ximopen(xw.dpy)) { XRegisterIMInstantiateCallback(xw.dpy, NULL, NULL, NULL, ximinstantiate, NULL); }
...
int xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y) {
    ...
    for (i = 0, xp = winx, yp = winy + font->ascent; i < len; ++i) {
        ...
        for (f = 0; f < frclen; f++) {
            ...
            if (!glyphidx && frc[f].flags == frcflags && frc[f].unicodep == rune) { break; }
        }
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y) {
    ...
    if (x == 0) { xclear(0, (y == 0) ? 0 : winy, borderpx, winy + win.ch + ((winy + win.ch >= borderpx + win.th) ? win.h : 0)); }
    if (winx + width >= borderpx + win.tw) { xclear(winx + width, (y == 0) ? 0 : winy, win.w, ((winy + win.ch >= borderpx + win.th) ? win.h : (winy + win.ch))); }
    ...
    if (base.mode & ATTR_UNDERLINE) { XftDrawRect(xw.draw, fg, winx, winy + dc.font.ascent + 1, width, 1); }

    if (base.mode & ATTR_STRUCK) { XftDrawRect(xw.draw, fg, winx, winy + 2 * dc.font.ascent * chscale / 3, width, 1); }
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len) {
    ...
    if (IS_SET(MODE_FOCUSED)) {
        switch (win.cursor) {
            ...
            case 4: /* Steady Underline */ XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + (cy + 1) * win.ch - cursorthickness, win.cw, cursorthickness); break;
            case 5:           /* Blinking bar */
            case 6: /* Steady bar */ XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + cy * win.ch, cursorthickness, win.ch); break;
        }
    } else {
...
reallocbuf:
    ...
    if (len <= buf_size) { ttywrite(buf, len, 1); }
cleanup:
    if (buf) { free(buf); }
}
...
int resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst) {
    ...
    switch (rtype) {
        case STRING: *sdst = ret.addr; break;
